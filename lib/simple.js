...
export function jidDecode(jid = '') {
  ...
  // Ejemplo básico, adaptalo si usas otros formatos:
  return jid.split(':')[0] + '@s.whatsapp.net';
}

export function createWASocket(connectionOptions, options = {}) {
  // 1) Creamos la conexión sin fallback legacy
  ...
  download: {
    async value(saveToFile = false) {
        ...
        try {
            ...
            console.error('❌ Error al descargar mensaje:', err)
            return null
        }
    },
    enumerable: true,
...
reply: {
    async value(text, chatId, options) {
        ...
        if (!conn) throw new Error('❌ Conexión no encontrada (conn)')
